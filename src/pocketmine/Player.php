	 ...
	 */
		...
		switch($packet::NETWORK_ID){
			...
			case ProtocolInfo::LOGIN_PACKET:
                ...
                $acceptedProtocols = array_merge(
                    ProtocolInfo::ACCEPTED_PROTOCOLS,
                    ProtocolInfo::ACCEPTED_013_PROTOCOLS
                );

				// if(!in_array($packet->protocol1, ProtocolInfo::ACCEPTED_PROTOCOLS)){
				if(!in_array($packet->protocol1, $acceptedProtocols)){
			...
			case ProtocolInfo::DROP_ITEM_PACKET:
				...
				if($dropItem->getId() === 0 or $dropItem->getCount() <= 0){
					$this->inventory->sendContents($this);
					break;
				}
	...
	public final function close($message = "", $reason = "generic reason", $notify = true){
		...
		if($this->connected and !$this->closed){
			if($notify and strlen((string) $reason) > 0){
				...
				$pk->message = $reason;
				$pk->encode();
				$this->interface->putPacket($this, $pk, false, true);
			}
