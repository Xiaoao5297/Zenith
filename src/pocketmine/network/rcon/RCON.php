...
class RCON{
	...
	public function __construct(Server $server, $password, $port = 19132, $interface = "0.0.0.0", $threads = 1, $clientsPerThread = 50){
		...
		$this->server->getLogger()->info("正在启用远程监听器");
		if($this->password === ""){
			//$this->server->getLogger()->critical("RCON can't be started: Empty password");
			$this->server->getLogger()->critical("RCON无法启动:空密码");
		...
		if($this->socket === false or !socket_bind($this->socket, $interface, (int) $port) or !socket_listen($this->socket)){
			...
			$this->server->getLogger()->critical("RCON无法启动: " . socket_strerror(socket_last_error()));
			$this->threads = 0;
			return;
		}
		...
		$this->server->getLogger()->info("RCON运行在 $addr:$port");
	}
