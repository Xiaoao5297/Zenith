...
class AIHolder {
	...
	public function RotationTimer() {
		foreach($this->getServer()->getLevels() as $level) {
			foreach($level->getEntities() as $entity) {
				if($entity instanceof Zombie or $entity instanceof Creeper or $entity instanceof Skeleton or $entity instanceof Cow or $entity instanceof Pig or $entity instanceof Sheep or $entity instanceof Chicken or $entity instanceof Spider) {
		...
		if($event instanceof EntityDamageByEntityEvent) {
			...
			if(isset($array[$entity->getId()])) {
				if($p instanceof Player and ($array[$entity->getId()]['canAttack'] == 0)) {
					...
					$targetX = $array[$entity->getId()]['x'] - $array[$entity->getId()]['xxx'] * $back;
					$targetY = $entity->getY() + $high;
					$targetZ = $array[$entity->getId()]['z'] - $array[$entity->getId()]['zzz'] * $back;
					$pos = new Vector3($targetX, $targetY, $targetZ);
					//如果目标击退位置被墙挡住，则缩减击退距离
					if(!$this->canKnockBackTo($entity->getLevel(), $pos)) {
						$back = max(0.5, $back / 2);
						$targetX = $array[$entity->getId()]['x'] - $array[$entity->getId()]['xxx'] * $back;
						$targetZ = $array[$entity->getId()]['z'] - $array[$entity->getId()]['zzz'] * $back;
						$pos = new Vector3($targetX, $targetY, $targetZ);
					...
					if($this->canKnockBackTo($entity->getLevel(), $pos)) {
						$array[$entity->getId()]['x'] = $targetX;
						$array[$entity->getId()]['y'] = $targetY;
						$array[$entity->getId()]['z'] = $targetZ;
