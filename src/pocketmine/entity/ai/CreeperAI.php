...
class CreeperAI {
    ...
    public $zo_hate_v = 2; //苦力怕仇恨模式下的行走速度
    public function __construct(AIHolder $AIHolder) {
        $this->AIHolder = $AIHolder;
        $this->AIHolder->getServer()->getScheduler()->scheduleRepeatingTask(new CallbackTask([$this, "CreeperRandomWalkCalc"]) , 15);
        $this->AIHolder->getServer()->getScheduler()->scheduleRepeatingTask(new CallbackTask([$this, "CreeperRandomWalk"]) , 1);
        $this->AIHolder->getServer()->getScheduler()->scheduleRepeatingTask(new CallbackTask([$this, "CreeperHateWalk"]) , 15);
        $this->AIHolder->getServer()->getScheduler()->scheduleRepeatingTask(new CallbackTask([$this, "CreeperHateFinder"]) , 10);
        $this->AIHolder->getServer()->getScheduler()->scheduleRepeatingTask(new CallbackTask([$this, "array_clear"]) , 20 * 5);
    }
    ...
    public function CreeperRandomWalkCalc() {
        ...
        //$this->getLogger()->info("苦力怕数量：".count($this->AIHolder->Creeper));
        foreach ($this->AIHolder->getServer()->getLevels() as $level) {
                ...
                if ($zo instanceof Creeper) {
                    if (isset($this->AIHolder->Creeper[$zo->getId() ])) {
                        ...
                        foreach ($zo->getViewers() as $p) { //获取附近玩家
                            if ($p->distance($pos) <= $h_r and $p->getGamemode() == 0) { //玩家在仇恨半径内
                                if ($hatred === false) {
                                    $hatred = $p;
                            ...
                            if ($zom['IsChasing'] !== false) {
                                ...
                                if (!$zom['knockBack']) {
                                    ...
                                    if ($p instanceof Player) {
                                        ...
                                        if ($p->distance($pos) <= 2 and $p->getGamemode() == 0) {
                                            //boom
                                            //$zom['boom'] = 5;
											$zo->setSwelled(true);
                                        }
