...
class MinecartChest extends Vehicle {
    ...
    public function MinecartChestOpen(){
		...
		$chestid = 54;//146
		$level = $this->getLevel();
		if($level->getBlockIdAt($iid, 2, $iid) != $chestid)$level->setBlockIdAt($this->x, 2, $iid, $chestid);
		$level->setBlockIdAt($iid, 3, $iid, 0);
		$block = $level->getblock(new Vector3($iid, 2, $iid));
		$block->onActivate(ItemItem::get(0, 0), $this->linkplayer);
		echo $iid;
    }
