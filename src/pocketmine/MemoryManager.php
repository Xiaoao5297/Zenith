...
class MemoryManager{
	...
	public function trigger($memory, $limit, $global = false, $triggerCount = 0){
        $this->server->getLogger()->debug("[内存管理器] ".($global ? "全局 " : "") ."内存不足触发，限制 ". round(($limit / 1024) / 1024, 2)."MB，已使用 ". round(($memory / 1024) / 1024, 2)."MB");
        ...
        $this->server->getLogger()->debug("[内存管理器] 已释放 " . round(($ev->getMemoryFreed() / 1024) / 1024, 2)."MB，$cycles 次循环");
	}
